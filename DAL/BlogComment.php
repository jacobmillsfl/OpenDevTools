...
class BlogComment {
	...
	public function __constructFull($paramId,$paramBlogId,$paramUserId,$paramComment,$paramCreateDate) {
		$this->id = $paramId;
	...
	public static function loadall() {
		...
		if ($result->num_rows > 0) {
			$arr = array();
			while ($row = $result->fetch_assoc()) {
				$blogComment = new BlogComment($row['id'],$row['blogId'],$row['userId'],$row['comment'],$row['createDate']);
				$arr[] = $blogComment;
			}
			return $arr;
		}
	...
	public static function search($paramId,$paramBlogId,$paramUserId,$paramComment,$paramCreateDate) {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_BlogComment_Search(?,?,?,?,?)');
		$arg1 = BlogComment::setNullValue($paramId);
		$arg2 = BlogComment::setNullValue($paramBlogId);
		$arg3 = BlogComment::setNullValue($paramUserId);
		$arg4 = BlogComment::setNullValue($paramComment);
		$arg5 = BlogComment::setNullValue($paramCreateDate);
		$stmt->bind_param('iiiss',$arg1,$arg2,$arg3,$arg4,$arg5);
		$stmt->execute();
